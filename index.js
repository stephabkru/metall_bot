...
//Наш секретный ключ для тестирования
const CODE = 'Металл';

//Хэдер для пост запроса на сервер
const headers = {
	'User-Agent':       'Super Agent/0.0.1',
    'Content-Type':     'application/json'
};

//Массив с объектами и признаками
const data_main = [
	{
		i: '1',
		text: 'Ст0',
...
// Самый первый шаг с запрашиванием кода
...
// Второй шаг с выводом списка, из элементов которого будет составлено тестирование
...
// Третий шаг, в котором мы составляем POST запрос для сервера, выдающего нам вопросы, отправляем запрос, получаем вопросы и записываем их базу данных.
...
// Шаг завершения теста, когда мы выводим процент проавильных ответов
...
//Шаг вывода вопроса пользователю
...
//Шаг прохождения самого тестирования
...
//Шаг вывода статистики или же перехода на новое тестирование
...
bot.use(function(message) {
	...
	let query = { 
  	...
  	User.findOneAndUpdate(query, update, options, function(error, user) {
  		...
  		//Проверяем, хочет ли пользователь стереть данные, но запрещаем ему делать это во всех случаях, кроме случая конца тестирования
